...
public:
    ...
    void move(double tau, bool flag) {

        if(flag){
            time-=tau;
            x -= vx * 4 * time * tau / pow((1 + pow(time, 2)), 2);
            y -= vy * tau;
            z -= vz * 2 * (1 - pow(time, 2)) * tau / pow((1 + pow(time, 2)), 2);
        }
        else {
            time+=tau;
            x += vx * 4 * time * tau / pow((1 + pow(time, 2)), 2);
    ...
    void smth_change(double tau){

        double k = 1;
        double w = 0.1;

        smth=1000000* cos(x * k - w*time);
    }
};
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = pow(pointX*10, 2) + pow(pointY*10, 2) + pow(pointZ*10, 2);
            double vz = 0;
            double vx = 0;
            if(pointX < middle_x && pointY > middle_y){
                vz = -abs(35 - pointX);
                vx = abs(35 - pointX);

            }
            nodes[i] = CalcNode(pointX, pointY, pointZ, smth, vx, 0.0, vz);
        }
    ...
    void doTimeStep(double tau, double dc, bool flag) {
        // По сути метод просто двигает все точки
        for(unsigned int i = 0; i < nodes.size(); i++) {
            nodes[i].move(tau, flag);
            nodes[i].smth_change(dc);
        }
    }
...
int main()
{
    ...
    // Считаем STL
    try {
        gmsh::merge("/home/ilya/science_prog_lab/vtk_lab/hand.stl");
    } catch(...) {
    ...
    for(unsigned int step = 1; step < 100; step++) {
        if(step>50) flag = true;
        mesh.doTimeStep(tau, dc, flag);
        mesh.snapshot(step);
    }
    return 0;
}
