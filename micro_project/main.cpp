...
// .. code-block:: cpp
template<typename T>
std::pair<std::vector<T>, std::vector<size_t>> f_interpolate_boundary(double t, auto& func){
    std::vector<T> f;
    for (std::size_t p = 0; p < func.x.extent(1); ++p)
    {
        auto pi = std::numbers::pi;
        f.push_back(t * 4.0 * std::pow(pi, 1) * std::sin(pi * func.x(0, p))
                    * std::sin(pi * func.x(1, p)));
    }

    return {f, {f.size()}};
}
std::pair<std::vector<T>, std::vector<size_t>> g_interpolate_boundary(double t, auto& func){
    std::vector<T> f;
    for (std::size_t p = 0; p < func.x.extent(1); ++p)
        f.push_back(t * std::sin(5 * func.x(0, p)));
    return {f, {f.size()}};
}
...

  {
    ...
    auto mesh = std::make_shared<mesh::Mesh>(
        mesh::create_rectangle(MPI_COMM_WORLD, {{{0.0, 0.0}, {2.0, 2.0}}},
                               {96, 96}, mesh::CellType::triangle, part));
    ...
    f->interpolate(
              [t](auto x) -> std::pair<std::vector<T>, std::vector<std::size_t>>
              {
                  std::vector<T> f;
                  for (std::size_t p = 0; p < x.extent(1); ++p)
                  {
                      auto pi = std::numbers::pi;
                      f.push_back(t * 4.0 * std::pow(pi, 1) * std::sin(pi * x(0, p))
                                  * std::sin(pi * x(1, p)));
                  }
    ...
    */
    ...
    // :cpp:class:`FunctiЫЫon` can be saved to a file. Here, we output the
